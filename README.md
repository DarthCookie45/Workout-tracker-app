...
Workout Tracker was developed to address common challenges faced by individuals who follow structured fitness programmes. Many gym users rely on memory, notebooks or spreadsheets to manage their training, which can make it difficult to maintain consistency and accurately monitor progression over time.

The application provides a centralised platform where users can create workout routines, schedule training sessions and record exercise information. By storing this data securely within a database, users can access their workout information from any device and avoid losing historical training records.

The progress tracking features provide additional value by allowing users to monitor performance trends, identify personal bests and view training statistics. This helps users make informed decisions about their training and maintain motivation through visible evidence of improvement.

The application's calendar and scheduling functionality also help users plan future workouts, improving consistency and supporting long-term fitness goals.
...
During testing, password reset requests successfully generated emails, however the password reset workflow displayed Django's default authentication templates rather than the application's custom user interface.

#### Investigation

Testing confirmed that the password reset functionality itself was working correctly. The issue was caused by Django's built-in authentication views not being configured to use the custom templates created for the project.

#### Resolution

Custom password reset templates were created and explicitly linked to the relevant Django authentication views within the URL configuration.

#### Outcome

The password reset workflow now provides a consistent user experience that matches the application's design and branding.
...
Following deployment to Heroku, the logout feature returned a "405 Method Not Allowed" error when users attempted to log out.

#### Investigation

Research identified that newer versions of Django require logout requests to be submitted using a POST request rather than a GET request for security reasons.

#### Resolution

The logout navigation link was replaced with a form that submits a POST request and includes CSRF protection.

#### Outcome

Users can now successfully log out in both development and production environments.

---

### Exercise Library Duplication

#### Issue

When editing an existing exercise using the exercise library, duplicate exercise records could be created instead of updating the original exercise.

#### Investigation

Testing revealed that the edit workflow was creating a new exercise instance rather than modifying the existing record.

#### Resolution

The edit logic was updated to overwrite the existing exercise record and preserve the relationship with the workout routine.

#### Outcome

Exercises can now be edited without creating duplicate records, improving data accuracy and user experience.
...
The weekly calendar view occasionally displayed an incorrect month when navigating between weeks.

#### Investigation

The calendar context was using the current date rather than the selected week date when determining which month should be displayed.

#### Resolution

The calendar context logic was updated to reference the selected week date when generating calendar information.

#### Outcome

The calendar now correctly displays the appropriate month while navigating between weeks.
...
HTML validation identified errors on the registration page caused by invalid nested markup generated through Django's default form rendering.

#### Investigation

Validation testing using the W3C Markup Validation Service highlighted structural HTML issues within the rendered registration form.

#### Resolution

The registration form was rewritten using individually rendered form fields and Bootstrap styling rather than relying on Django's default form output.

#### Outcome

The registration page now validates successfully and provides greater control over styling, accessibility and user feedback.
